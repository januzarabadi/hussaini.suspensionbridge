...
${suffix}`;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const createReviewCard = (review, index = 0) => {
      ...
      const quote2 = document.createElement("blockquote");
      ...
      quote2.textContent = `\u201C${review.review.trim()}\u201D`;
      avatar.className = `avatar${avatarClass ? ` ${avatarClass}` : ""}`;
      avatar.textContent = getInitials(name);
      author.textContent = name;
      meta.textContent = `${review.country.trim()} \xB7 ${formatDate(review.created_at)}`;
      details.append(author, meta);
      caption.append(avatar, details);
      figure.append(stars, quote2, caption);
      return figure;
    };
    ...
    const loadReviews = async () => {
      const { data, error } = await client.from("review").select("id, name, country, rating, review, created_at").order("created_at", { ascending: false });
      loading?.remove();
      if (error) {
    ...
    form.addEventListener("submit", async (event) => {
      ...
      const { data, error } = await client.from("review").insert({
        name: name || null,
        country,
        rating,
        review: reviewText
      }).select("id, name, country, rating, review, created_at").single();
